...
// Swaps in a user-supplied key (see server/lib/userKeys.js) for a single
// request, WITHOUT mutating the module-level PROVIDERS array — each call
// gets its own shallow-cloned provider object, so one browser's custom key
// never bleeds into another's request even under concurrent traffic.
// Cloudflare is the odd one out (its baseURL bakes in the account id), so
// it gets its own branch; everything else is just apiKey.
function withUserOverride(provider, userKeys) {
  const o = userKeys && userKeys[provider.name];
  if (!o) return provider;
  if (provider.name === "cloudflare") {
    const accountId = o.accountId || process.env.CLOUDFLARE_ACCOUNT_ID;
    const apiToken = o.apiToken || provider.apiKey;
    if (!accountId || !apiToken) return provider;
    return {
      ...provider,
      apiKey: apiToken,
  ...
  if (!o.apiKey) return provider;
  return { ...provider, apiKey: o.apiKey };
}
 ...
 *
 * `userKeys` (optional) is the calling browser's own saved API keys (Dev
 * Mode -> "API key sendiri"; see server/lib/userKeys.js) — when present for
 * a given provider it's used INSTEAD OF this server's .env key for that
 * provider, letting someone run the app on their own quota without editing
 * .env. Never logged; see userKeys.js for how it's stored.
 */
async function chat({ messages, tools, temperature, max_tokens, forceProvider, userKeys }) {
  const resolved = PROVIDERS.map((p) => withUserOverride(p, userKeys));
  let active = resolved.filter((p) => p.alwaysAvailable || p.apiKey);
  ...
  if (forceProvider) {
    ...
    const only = resolved.find((p) => p.name === forceProvider);
