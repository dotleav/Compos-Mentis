...
/** Path to one converted case's JSON (compositeId, e.g. "ab12cd34__end_08")
 * — used for the optional per-user Drive backup (server/routes/customCases.js),
 * so the converted case ends up in the user's own Drive, not just the
 * original docx. */
function getCaseJsonPath(compositeId) {
  const p = path.join(CASES_DIR, CUSTOM_KATEGORI, `${compositeId}.json`);
  return fs.existsSync(p) ? p : null;
}
