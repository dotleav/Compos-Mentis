...
async function main() {
  ...
  // 3. AI draft
  const { chat, PROVIDERS } = require("../server/lib/providers");
  if (!PROVIDERS.some((p) => p.alwaysAvailable || p.apiKey)) {
    console.error(
      "No AI provider configured — cannot use --ai. Set GROQ_API_KEY, CEREBRAS_API_KEY, or OPENROUTER_API_KEY in .env first (or run Ollama locally)."
    );
    process.exit(1);
  }
  const schema = fs.readFileSync(path.join(ROOT, "data", "cases", "_SCHEMA.md"), "utf-8");

  console.log("\nAsking the AI provider to draft case JSON — this may take a bit for large tables...");
  const response = await chat({
    messages: [
      {
        role: "system",
        content: `Kamu membantu mengubah tabel kasus OSCE mentah menjadi JSON terstruktur sesuai skema berikut. Jangan mengubah/menambah fakta medis apa pun dari teks asli — hanya restrukturisasi ke format JSON. Jika suatu field tidak ada di teks asli, isi dengan string kosong atau array kosong, JANGAN mengarang. Keluarkan HANYA JSON array valid (array of case objects sesuai skema), tanpa teks lain, tanpa markdown code fences.\n\nSKEMA:\n${schema}`,
      },
      {
        role: "user",
  ...
  console.log(`(drafted using provider: ${response._provider})`);
