...
async function showLanding() {
  ...
  app.innerHTML = `
    ...
    <div class="card" id="customCaseCard">
      ...
      <p class="muted" style="margin-bottom:14px; font-size:0.85rem;">
        Belum punya file-nya? <a href="/api/custom-cases/template" id="downloadTemplateLink">Download template kasus custom di sini</a>.
      </p>
  ...
  function renderDriveConnectPrompt() {
    driveSection.innerHTML = `
      <p class="muted" style="font-size:0.85rem; margin-bottom:10px;">Hubungkan Google Drive-mu sendiri supaya setiap docx yang kamu upload, beserta hasil kasus JSON-nya, otomatis dibackup ke folder di Drive-mu (bukan disimpan di server) — folder dibuat otomatis, tidak perlu dikonfigurasi.</p>
  ...
  function renderDriveConnected(folder) {
    driveSection.innerHTML = `
      <p class="muted" style="font-size:0.85rem;">Tersambung ke folder Drive milikmu sendiri: <b style="color:var(--text);">${folder.name}</b></p>
  ...
  async function renderDriveSection() {
    ...
    // A folder is created automatically the moment the connection succeeds
    // (see server/lib/googleDrive.js) — so "connected" and "has a folder"
    // happen together; no separate paste-a-folder step needed.
    if (!status.configured) renderDriveNotConfigured();
    else if (!status.connected || !status.folder) renderDriveConnectPrompt();
